...
class TestComponent extends Component {
  ...
  handleTestUpdateProfile = async () => {
    ...
    let userDocRef = await firestore
      .collection('users')
      .doc('L9FgckWQCiS0PCaGaRPqb8Yu6gz1'); // Jess
    try {
  ...
  handleTestJoinEvent = async () => {
    ...
    try {
      await eventDocRef.update({
        [`attendees.L9FgckWQCiS0PCaGaRPqb8Yu6gz1`]: attendee,
      });
      toastr.success('Success');
    } catch (error) {
      console.log(error);
  ...
  handleTestCancelGoingToEvent = async () => {
    ...
    try {
      await eventDocRef.update({
        [`attendees.L9FgckWQCiS0PCaGaRPqb8Yu6gz1`]: firebase.firestore.FieldValue.delete(),
      });
      toastr.success('Success');
    } catch (error) {
      console.log(error);
  ...
  handleTestChangeAttendeePhotoInEvent = async () => {
    ...
    try {
      await eventDocRef.update({
        [`attendees.L9FgckWQCiS0PCaGaRPqb8Yu6gz1.photoURL`]: 'testing123.jpg',
      });
      toastr.success('Success');
    } catch (error) {
      console.log(error);
